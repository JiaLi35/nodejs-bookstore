...
app.get("/books/:id", (req, res) => {
  ...
  const selectedBooks = books.find((b) => b.id === book_id);
...
app.get("/reviews/:id", (req, res) => {
  ...
  const selectedReviews = reviews.find((r) => r.id === review_id);
...
app.get("/authors/:id", (req, res) => {
  ...
  const selectedAuthor = authors.find((a) => a.id === author_id);
  res.send(selectedAuthor);
});

app.listen(5124, () => {
  console.log("Bookstore app is running on http://localhost:5124");
});
